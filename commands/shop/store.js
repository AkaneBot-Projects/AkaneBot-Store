  ...
  execute: async function (m, { args, command, text, client, Func }) {
    ...
    switch (command) {
      ...
      case "updatelist":
        ...
        const existingItem = getDataResponList(
          groupId,
          keyToUpdate.trim(),
          db_store,
        );
      ...
      case "proses":
        ...
        const existingProcessItem = getDataResponList(
          groupId,
          keyToProcess,
          db_store,
        );
        ...
        const processReason =
          args.slice(1).join(" ") || "Pesanan sedang diproses";
...
╰── ♡ ⋆｡°✩ ──╯`;
        ...
        if (
          ...
          await m.reply({
            image: { url: existingProcessItem.image_url },
      ...
      case "done":
        ...
        const existingDoneItem = getDataResponList(
          groupId,
          keyToDone,
          db_store,
        );
...
ᵀᵉʳⁱᵐᵃᵏᵃˢⁱʰ ᵗᵉˡᵃʰ ᵇᵉʳᵇᵉˡᵃⁿʲᵃ! ˢⁱˡᵃʰᵏᵃⁿ ᵈᵃᵗᵃⁿᵍ ᵏᵉᵐᵇᵃˡⁱ ❤️`;
        ...
        if (existingDoneItem.isImage && existingDoneItem.image_url !== "-") {
          await m.reply({
            image: { url: existingDoneItem.image_url },
            caption: doneMessage,
          });
        } else {
