export default {
  ...
  execute: async function (
    m,
    { client, command, prefix, args, text, quoted, options },
  ) {
    try {
      switch (command) {
        case "topup":
...
Status: ${orderResponse.data.status}`);
        ...
        case "cekservice":
          ...
          const uniqueCategories = [
            ...new Set(
              servicesResponse.data.map((service) => service.kategori),
            ),
          ];
...
Contoh: ${prefix}service "Mobile Legends"`);
          ...
          const filteredServices = allServicesResponse.data.filter(
            (service) =>
              service.kategori.toLowerCase() === serviceCategory.toLowerCase(),
          );

          if (filteredServices.length === 0) {
            return m.reply(
              `❌ Tidak ada layanan ditemukan untuk kategori "${serviceCategory}"`,
            );
          }
...
Keterangan: ${statusResponse.data.keterangan || "Tidak ada keterangan"}`);
  ...
  after: async function (m) {
    console.log(
      `User ${m.sender} menggunakan perintah TopupInyuk: ${m.command}`,
    );
  },
};
